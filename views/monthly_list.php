...
<?php } ?>
    <div class="table-responsive">
      <form action=".?menu=monthly-list" method="POST">
        <table class="table table-striped table-bordered align-middle col2-align">
          <thead>
            <tr class="text-center row-nowrap">
              <th>
                月
                <input id="purchase-month-asc" class="btn-check" type="submit" name="order" value="1 asc" autocomplete="off"<?= $order === '1 asc' ? ' checked' : '' ?>>
                <label class="btn-sm btn-secondary" for="purchase-month-asc">▲</label>
                <input id="purchase-month-desc" class="btn-check" type="submit" name="order" value="1 desc" autocomplete="off"<?= $order === '1 desc' ? ' checked' : '' ?>>
